...
def main():
    print("=== EEGNet PRE-TRAINING (Physionet 3-Class) ===")
    
    # CACHE PATH
    # DATASET (Frozen 3-Class)
    CACHE_FILE = os.path.join(os.path.dirname(__file__), "PRETRAIN_DATABASE_3CLASS.npz")
    
    # OUTPUT DIRS
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    run_dir = os.path.join(os.path.dirname(__file__), "runs", f"pretrain_3class_{timestamp}")
    ...
    if os.path.exists(CACHE_FILE):
        ...
        # print("   Please run 'python tools/build_physionet_3class.py' first.")
        return
    ...
    try:
        for epoch in range(n_epochs):
            ...
            if val_loss < best_val_loss:
                ...
                best_epoch = epoch + 1
                best_model_path = os.path.join(run_dir, "eegnet_physionet_best_3class.pth")
            ...
            plt.tight_layout()
            save_path = os.path.join(run_dir, 'pretrain_metrics_3class.png')
    ...
    model_path = os.path.join(run_dir, "eegnet_physionet_weights_3class.pth")
    torch.save(model.state_dict(), model_path)
    print(f"✅ Final Model saved to {model_path}")

    # FINAL MESSAGE
    print(f"\n✅ Pre-training (3-Class) Complete!")
    print(f"   Shape: {X.shape}")
    print(f"   Best Model: {best_model_path}")
    print(f"   Final Model: {model_path}")
    print(f"👉 To Finetune: Update 'WEIGHTS_FILE' in finetune_eegnet_aug_3class.py with the path above.")
